...
namespace SimulationTask1
{
    public partial class tableAndCharts : Form
    {
        ...
        //b-plot el table 
        public void simulate(int customerNumber, double rnd1, double interArrivalTime, double arrivalTime, double serverIndex,
            double timeServiceBegin, double rnd2, double serviceDur, double timeServiceEnd, double totalDelay)
        {
        ...
        public void plotChart(double end, double[] timeServiceBegin, double[] arrivalTime,int numCust)
        {
            ...
            chart1.ChartAreas[0].AxisX.Maximum = end+1;
        ...
        public void plotDelayChart(int NumCustomers,double [] DelayDur)
        {
            double min = DelayDur.Min();
            double max = DelayDur.Max() + 1;
            ...
            for (int i = 0; i <= max; i++)
            {
                ...
                for(int j =1; j < NumCustomers ;j++)
                {
                    if (DelayDur[j] > 0)
                    {
                        if (DelayDur[j] >= i && DelayDur[j] <= i + 1)
                        {
                            numCust++;
                        }
                        double first = i + 0.5;
